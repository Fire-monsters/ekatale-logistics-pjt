    ...
    >
      ...
      >
        ...
        <View style={styles.headerBlock}>
          <Text style={styles.pageEmoji}>{isFarmer ? '📱' : '🧑‍💼'}</Text>
          <Text style={styles.pageTitle}>Phone &amp; Password</Text>
          <Text style={styles.pageSubtitle}>
            {`Creating account for ${draft.fullName ?? (isFarmer ? 'Farmer' : 'Field Agent')}`}
          </Text>
        </View>
                  ...
                  >
                    ...
                    {payProvider === p.key && (
                      <View style={styles.payCheck}>
                        <Text style={styles.payCheckText}>✓</Text>
                      </View>
                    )}
                  </TouchableOpacity>
                ))}
...
const styles = StyleSheet.create({
  ...
  payCheck:      { width: 20, height: 20, borderRadius: 10, backgroundColor: Colors.green, alignItems: 'center', justifyContent: 'center' },
  payCheckText:  { fontSize: 11, color: Colors.textInverse, fontWeight: Font.weight.bold },
